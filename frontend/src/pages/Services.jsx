                    ...
                    {(isLoading) ? (<div>{t("loading")}</div>) : (isError) ? (<div>{t("services.errorLoadingUsers")}</div>) :
                        (DBservices[0] === "start") ? null :
                            (isSuccess && DBservices?.length === 0) ?
                                (<div className="flex justify-center gap-2">{t("services.noProvidersFound")}<span className="text-primary">{t(`fields.${service}`)}</span>  {t("services.in")}<span className="text-primary">{t(`cities.${city}`)}</span>   😓</div>)
                                : (
                                    <div>
