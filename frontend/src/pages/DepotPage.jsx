...
import { Bookmark, Cookie, Loader, MapPin, PackageOpen, Search, ShoppingBasket, SlidersHorizontal, TriangleAlert, X } from "lucide-react";
		...
		<>
			...
			<div className='bg-body px-4 py-6 grid grid-cols-1 lg:grid-cols-4 gap-6'>
				...
				<div className='col-span-1 lg:col-span-2'>
						...
						<div>
							<div className="flex">
								<div className="bg-gray-100 border border-gray-300 rounded-lg w-full relative ">
									<input type="text" className="w-full bg-gray-100 py-2 px-4 rounded-lg" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("depot.searchPlaceholder")} />
									<Search className="text-gray-500 absolute right-4 top-1/2 transform -translate-y-1/2" />
								</div>
								<button className="btn border border-gray-300 ml-4 sm:hidden" onClick={() => setIsfilterOpen(!isfilterOpen)}>
									{isfilterOpen ? <X size={18} /> : <SlidersHorizontal size={18} />}
								</button>
							</div>
							{(filter.item !== "all" || filter.city !== "all" || filter.category !== "all" || filter.itemStatus !== "all") && (
								<p className="text-gray-500 text-sm flex items-center gap-1 mt-1"> <TriangleAlert />{t("depot.applyingFilter")}</p>
							)}					</div>
						{isfilterOpen && (
							<div className='bg-white sm:hidden p-4 mt-2'>
								<div className="flex justify-between items-center">
									<h2 className='font-semibold flex items-center gap-2'><SlidersHorizontal size={18} /><span>{t("depot.filter")}</span></h2>
									<button className="text-red-400 text-sm font-semibold" onClick={() => resetFilter()}> {t("depot.reset")}</button>
								</div>

								<div className="flex flex-col gap-2 mb-2 mt-4 sm:mt-8">
									<label htmlFor="item" className="text-gray-500 text-sm flex items-center gap-1"><ShoppingBasket size={15} className="text-orange-300" /><span>{t("depot.item")}</span> </label>
									<select id="item" name="item" className="flex-1 border rounded p-1"
										value={filter.item}
										onChange={(e) => setFilter({ ...filter, item: e.target.value })}>
										<option value="all">{t("depot.all")}</option>
								...
								<div className="flex flex-col gap-2  mt-4">
									<label htmlFor="location" className="text-gray-500 text-sm flex items-center gap-1"><MapPin size={15} className="text-orange-300" /><span>{t("depot.location")}</span></label>
								...
								<div className="flex flex-col gap-2  mt-4">
									<label htmlFor="category" className="text-gray-500 text-sm flex items-center gap-1"><Bookmark size={15} className="text-orange-300" /><span>{t("depot.category")}</span></label>
								...
								<div className="flex flex-col gap-2  mt-4">
									<label htmlFor="item-status" className="text-gray-500 text-sm flex items-center gap-1"><Cookie size={15} className="text-orange-300" /><span> {t("depot.itemStatus")}</span></label>
					...
					<div ref={loadMoreRef}>
						{isFetchingNextPage && <p>{t("depot.loadingMoreScraps")}</p>}
					</div>
					...
					{filteredScraps?.length === 0 && !scrapLoading && (
						<div className='bg-white rounded-lg shadow p-8 text-center'>
							...
							<h2 className='text-2xl font-bold mb-4 text-gray-800'>{t("depot.noScraps")}</h2>
							<p className='text-gray-600 mb-6'>{t("depot.noScrapsDescription")}</p>
							<Link to="/create-scrap" className='text-sm font-semibold'>
								<button className="btn bg-primary text-white">{t("depot.createScrap")}</button>
							</Link>
						</div>
					)}
