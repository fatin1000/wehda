    ...
    <div className='relative w-full '>
      {scrapLoading ? (<div className="flex flex-col items-center mt-10 text-primary"><Loader className="mr-2 animate-spin" size={40} /></div>) : (<div>
        {(myScrapsArr.length === 0) ? (
          <div className="flex flex-col items-center mt-10">
            <p className="text-center text-2xl text-gray-500 font-semibold">{t('dashboard.myScraps.empty')}</p>
            <Link to="/create-scrap" className="bg-primary text-white py-3 px-9 rounded text-center mt-10 font-semibold hover:bg-orange-700">{t('dashboard.myScraps.makeOneNow')}</Link>
          </div>
                ...
                <div className="flex justify-between gap-2 mb-4">
                  {scrap.scrapStatus === "open" ? (
                    <div className="flex items-center gap-2">
                      <span className="bg-green-100 text-green-500 py-1 px-2 rounded-full text-sm">{t('dashboard.myScraps.open')}</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <span className="bg-gray-300 text-gray-600 py-1 px-2 rounded-full text-sm">{t('dashboard.myScraps.closed')}</span>
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <div className="flex flex-col gap-2">
                      <h3 className="text-2xl font-semibold text-orange-500">
                        {t(`items.${scrap.itemName.value}`)}</h3>
      ...
      <>
        ...
        {t(`units.${scrap.units}`)}
                      ...
                      <p className="text-gray-500 ">{t("scrap.unitPrice")} : {scrap.unitPrice} SR</p>
                      <div className="flex items-center gap-2">
                        {scrap.likes.length > 0 && (
                          <div className="flex items-center">
                            <span className="text-blue-500">{t('dashboard.myScraps.likesCount', { count: scrap.likes.length })}</span>
                          </div>
                        )}
                        {scrap.comments.length > 0 && (
                          <div className="flex items-center">
                            <span className="text-blue-500">{t('dashboard.myScraps.commentsCount', { count: scrap.comments.length })}</span>
                          </div>
                        )}
                      </div>
                    </div>
