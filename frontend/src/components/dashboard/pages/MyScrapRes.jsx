      ...
      {scrapResLoading ? (
        ...
        <div className="flex flex-col items-center mt-10">
          <p className="text-center text-2xl text-gray-500 font-semibold">
            {t("dashboard.myScrapsRequest.empty")}
          </p>
          <Link
            to="/depot"
          ...
          >
            {" "}
            {t("dashboard.myScrapsRequest.makeOneNow")}
          </Link>
        </div>
      ) : (
        <div>
          {scrapResLoading ? (
            ...
            scrapResponse.map((e) => (
              <div
                key={e._id}
                className="flex flex-col-reverse md:flex-row md:justify-between md:items-center gap-2 flex-wrap p-4 border rounded-lg shadow-md mb-4"
              >
                <div className="flex flex-col gap-2 md:flex-row">
                  <div className="flex-shrink-0  md:size-40">
                    <img
                      src={e.scrap.image}
                      alt={e.scrap.itemName.value}
                      className="rounded size-full"
                    />
                  </div>
                  <div>
                    <Link
                      to={`/scrap/${e.scrap._id}`}
                      className="font-medium text-orange-500 md:text-2xl"
                    >
                      {t(`items.${e.scrap.itemName.value}`)}
                    </Link>
                    <div className="my-2">
                      <Link
                        to={`/profile/${e.recipient._id}`}
                      ...
                      >
                        <div className="flex-shrink-0 h-10 w-10">
                          <div className="size-8 rounded-full overflow-hidden">
                            <img
                              src={
                                e.recipient.profilePic
                                  ? e.recipient.profilePic
                                  : "/avatar.png"
                              }
                              alt={e.recipient.username}
                            />
                          </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-2 md:flex-row">
                      <div className="flex items-center">
                        <p className="text-gray-500">{t("label.quantity")} : </p>
                        <span className="px-2 inline-flex  leading-5 font-semibold ">
                          {e.quantity}
                        </span>
                      </div>
                      <div className="flex items-center">
                        <p className="text-gray-500">{t("label.price")} : </p>
                        <span className="px-2 inline-flex  leading-5 font-semibold ">
                          {e.price} SR
                        </span>
                      </div>
