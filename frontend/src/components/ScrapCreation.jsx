...
const ScrapCreation = () => {
	...
	const translatedItemOptions = itemOptions.map(option => ({
		value: option.value,
	...
	const { mutate: createScrapMutation, isPending } = useMutation({
		...
		onSuccess: () => {
			resetForm();
			toast.success(t("scrap.createSuccess"));
			queryClient.invalidateQueries({ queryKey: ["scraps"] });
		},
		onError: (err) => {
			toast.error(err.response.data.message || t("scrap.createError"));
		},
	});
		...
		<form onSubmit={handleScrapCreation}>
			<div className="max-w-[90%] mx-auto mt-10 sm:max-w-[80%] ">
				<div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
					{/* step 1 */}
					<h3 className="text-1xl mb-4 font-semibold leading-6 text-primary sm:col-span-6 text-3xl">{t('form.title')}</h3>
					<div className="sm:col-span-4 flex gap-2 items-center">
						<label htmlFor="itemName" className='min-w-[100px] text-sm font-medium text-gray-700'>{t("form.itemName")}</label>
						<Select
							name="itemName"
							id="itemName"
							options={translatedItemOptions}
							value={itemName}
					...
					<div className="sm:col-span-3 flex gap-2 items-center">
						...
						<div className="mt-2 grid grid-cols-1">
							...
							>
								...
								{unitsOptions.map(e => <option key={e.value} value={e.value}>{t(e.label)}</option>)}
							</select>
							<ChevronDownIcon
								aria-hidden="true"
					...
					<div className="sm:col-span-2 flex gap-2 items-center ">
						...
						<div className="mt-2 rounded-md shadow-sm min-w-[100px] w-full">
							...
							>
								<option value="" disabled>{t("form.select")}.</option>
								{sellOptions.map(e => <option key={e.value} value={e.value}>{t(e.label)}</option>)}
							</select>
						</div>
					</div>
					...
					<div className="sm:col-span-6 flex gap-2 items-center">
						...
						<div className="mt-2 grid grid-cols-1">
							...
							>
								...
								<option value="" disabled>{t("form.select")}.</option>
								{categoryOptions.map(e => <option key={e.value} value={e.value}>{t(e.label)}</option>)}
							</select>
							<ChevronDownIcon
								aria-hidden="true"
								className="pointer-events-none col-start-1 row-start-1 mr-2 size-5 self-center justify-self-end text-gray-500 sm:size-4"
					...
					<div className="sm:col-span-4 ">
						<label htmlFor="discription" className='block text-sm font-medium text-gray-700'>{t("form.description")}</label>
						<textarea
							id="discription"
							name="discription"
							value={discription}
					...
					<div className="sm:col-span-4 flex gap-2 items-center">
						...
						<div className="mt-2 grid grid-cols-1">
							...
							>
								<option value="" disabled>{t("form.select")}.</option>
								{itemStatusOptions.map(e => <option key={e.value} value={e.value}>{t(e.label)}</option>)}
							</select>
							<ChevronDownIcon
								aria-hidden="true"
								className="pointer-events-none col-start-1 row-start-1 mr-2 size-5 self-center justify-self-end text-gray-500 sm:size-4"
				...
				<button
					...
					disabled={isPending}
				>
					{isPending ? <Loader className='size-5 animate-spin' /> : t("form.share")}
				</button>
			</div>
		</form>
	);
